...
if_stmt         : IF  expr  THEN  stmt_list  ELSE  stmt_list  END   { Debug("if_stmt -> if expr THEN stmt_lst ELSE stmt_list END ;"          ); $$ = ifstmt____IF_expr_THEN_stmtlist_ELSE_stmtlist_END($1,$2,$3,$4,$5,$6,$7); }
                ;

while_stmt      : WHILE  expr  BEGIN  stmt_list  END                { Debug("while_stmt -> while expr BEGIN stmt_lst END ;"          );        $$ = whilestmt____WHILE_expr_BEGIN_stmtlist_END($1,$2,$3,$4,$5); }

compound_stmt   : BEGIN  local_decls                            { Debug("compound_stmt -> begin local_decls"); $<obj>$ = compoundstmt____BEGIN_localdecls_10X_stmtlist_END($1, $2);}
                        stmt_list  END                { Debug("                      stmt_list END;"           );   $$ = compoundstmt____BEGIN_localdecls_X10_stmtlist_END($1,$2, $3, $4, $5); }
